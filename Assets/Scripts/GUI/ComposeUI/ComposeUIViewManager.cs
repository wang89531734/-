...
public class ComposeUIViewManager : MogoUIBehaviour
{
    ...
    {
        float fOffect = m_listIconGrid[id].transform.Find("EquipmentUIComposeIconListGridChildList").childCount * ICONGRIDCHILDSPACE * 0.0008f;
        float fScale = 0.001f;
        ...
        {
            fOffect = -m_listIconGrid[id].transform.Find("EquipmentUIComposeIconListGridChildList").childCount * ICONGRIDCHILDSPACE * 0.0008f;
            fScale = 1f;
        }
        else
        {
        ...
        AssetCacheMgr.GetUIInstance("EquipmentUIComposeIconListGridChild.prefab", (prefab, guid, go) =>
            {
                ...
                obj.transform.localPosition = new Vector3(0, -(obj.transform.parent.childCount - 1) * ICONGRIDCHILDSPACE, 0);
                obj.GetComponentsInChildren<MyDragCamera>(true)[0].RelatedCamera = m_dragCamera;
    ...
    public int GetIconGridChildNum(bool isOpened = true)
    {
        ...
        if (isOpened)
        {
            for (int i = 0; i < m_listIconGrid.Count; ++i)
            {
                ...
                {
                    num += m_listIconGrid[i].transform.Find("EquipmentUIComposeIconListGridChildList").childCount;
                }
            }
        }
        else
